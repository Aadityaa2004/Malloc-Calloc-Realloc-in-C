...
have also performed extra credit
...
void* mm_malloc(size_t size) {
    ...
    // Only split if the remainder would be at least MIN_BLOCK_SIZE
    if (blockSize >= reqSize + MIN_BLOCK_SIZE) {
        BlockInfo* newBlock;
        size_t newBlockSize = blockSize - reqSize;
        
        // Set up the remainder block
        newBlock = (BlockInfo*)UNSCALED_POINTER_ADD(ptrFreeBlock, reqSize);
        newBlock->sizeAndTags = newBlockSize | TAG_PRECEDING_USED;
        *((size_t*)UNSCALED_POINTER_ADD(newBlock, newBlockSize - WORD_SIZE)) = newBlockSize | TAG_PRECEDING_USED;
...
void mm_free(void* ptr) {
    ...
    // Clear the used tag and keep preceding used tag
    blockInfo->sizeAndTags &= ~TAG_USED;
    
    // Set up the boundary tag
    *((size_t*)UNSCALED_POINTER_ADD(blockInfo, size - WORD_SIZE)) = blockInfo->sizeAndTags;
    
    // Update the following block's preceding used tag
    BlockInfo* followingBlock = (BlockInfo*)UNSCALED_POINTER_ADD(blockInfo, size);
...
void* mm_realloc(void* ptr, size_t size) {
    ...
    size_t newReqSize = size + WORD_SIZE;
    newReqSize = ALIGNMENT * ((newReqSize + ALIGNMENT - 1) / ALIGNMENT);

    // If shrinking and the remaining size would be at least MIN_BLOCK_SIZE
    if (oldSize >= newReqSize + MIN_BLOCK_SIZE) {
        // Split the block
        size_t excessSize = oldSize - newReqSize;
        BlockInfo* newBlock = (BlockInfo*)UNSCALED_POINTER_ADD(blockInfo, newReqSize);
        
        // Set up the new free block
        newBlock->sizeAndTags = excessSize | TAG_PRECEDING_USED;
        *((size_t*)UNSCALED_POINTER_ADD(newBlock, excessSize - WORD_SIZE)) = 
            excessSize | TAG_PRECEDING_USED;
        
        // Update current block
        blockInfo->sizeAndTags = newReqSize | (blockInfo->sizeAndTags & TAG_PRECEDING_USED) | TAG_USED;
    ...
    BlockInfo* nextBlock = (BlockInfo*)UNSCALED_POINTER_ADD(blockInfo, oldSize);
    while (!(nextBlock->sizeAndTags & TAG_USED)) {
        printf("Found a free next block, checking if it can be used...\n");
        size_t nextBlockSize = SIZE(nextBlock->sizeAndTags);
        if (oldSize + nextBlockSize >= newReqSize) {
            // Remove next block from free list
            removeFreeBlock(nextBlock);
            printf("Successfully removed next block from free list\n");
            
            // Merge the blocks
            blockInfo->sizeAndTags = (oldSize + nextBlockSize) | (blockInfo->sizeAndTags & TAG_PRECEDING_USED) | TAG_USED;
            printf("Successfully merged blocks\n");
            
            // Split if possible
            if (oldSize + nextBlockSize >= newReqSize + MIN_BLOCK_SIZE) {
                BlockInfo* splitBlock = (BlockInfo*)UNSCALED_POINTER_ADD(blockInfo, newReqSize);
                size_t splitSize = oldSize + nextBlockSize - newReqSize;
                
                splitBlock->sizeAndTags = splitSize | TAG_PRECEDING_USED;
                *((size_t*)UNSCALED_POINTER_ADD(splitBlock, splitSize - WORD_SIZE)) = splitSize | TAG_PRECEDING_USED;
                
                blockInfo->sizeAndTags = newReqSize | (blockInfo->sizeAndTags & TAG_PRECEDING_USED) | TAG_USED;
                
                insertFreeBlock(splitBlock);
                printf("Successfully split block and inserted free portion\n");
            } else {
            ...
            printf("Error: Next block not large enough for reallocation\n");
            break;
        }
    }
